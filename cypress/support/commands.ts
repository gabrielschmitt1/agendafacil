...
// Custom Commands para AgendaFacil
...
Cypress.Commands.add('verificarAlert', (mensagem: string) => {
  cy.get('@alertStub').should('have.been.calledWithMatch', new RegExp(mensagem.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
});
...
Cypress.Commands.add('preencherFormularioAgendamento', (dados: AgendamentoData) => {
  ...
  // Campos obrigatórios
  cy.get('[data-testid="input-cliente-nome"]')
    .should('be.visible')
    .clear()
    .type(dados.clienteNome);
  ...
  cy.get('[data-testid="input-valor"]')
    .should('be.visible')
    .clear()
    .type(dados.valor);
  ...
  if (dados.custo) {
    cy.get('[data-testid="input-custo"]')
      .should('be.visible')
      .clear()
      .type(dados.custo);
  }
...
Cypress.Commands.add('navegarParaAba', (aba: 'NovoAgendamento' | 'MeusAgendamentos' | 'Relatorios' | 'Configuracoes') => {
  const abaMap = {
...
Cypress.Commands.add('criarAgendamento', (dados: AgendamentoData) => {
  ...
  cy.get('[data-testid="button-agendar"]')
    .should('be.visible')
    .click();
});
...
Cypress.Commands.add('buscarAgendamento', (termo: string) => {
  cy.get('[data-testid="searchbar-agendamentos"]')
    .should('be.visible')
    .clear()
    .type(termo);
...
Cypress.Commands.add('abrirMenuAgendamento', (nomeCliente: string) => {
  cy.get(`[data-testid="menu-button-${nomeCliente}"]`)
    .should('be.visible')
    .click();
});
...
Cypress.Commands.add('concluirAgendamento', (nomeCliente: string) => {
  ...
  cy.get('[data-testid="menu-option-concluir"]')
    .should('be.visible')
    .click();
});
...
Cypress.Commands.add('cancelarAgendamentoViaMenu', (nomeCliente: string) => {
  ...
  cy.get('[data-testid="menu-option-cancelar"]')
    .should('be.visible')
    .click();
});
...
Cypress.Commands.add('verDetalhesAgendamento', (nomeCliente: string) => {
  ...
  cy.get('[data-testid="menu-option-ver-detalhes"]')
    .should('be.visible')
    .click();
});
...
Cypress.Commands.add('alternarPeriodoRelatorio', (periodo: 'Semanal' | 'Mensal' | 'Anual') => {
  ...
  cy.get(`[data-testid="${periodoMap[periodo]}"]`)
    .should('be.visible')
    .click();
});
