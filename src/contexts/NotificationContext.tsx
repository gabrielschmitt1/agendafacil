...
const NOTIFICACOES_KEY = '@agendafacil:notificacoes';
const LEMBRETE_KEY = '@agendafacil:lembrete';
  ...
  const cancelarNotificacao = async (agendamentoId: string) => {
    try {
      const notificationId = await AsyncStorage.getItem(
        `@agendafacil:notification:${agendamentoId}`
      );

      if (notificationId) {
        await Notifications.cancelScheduledNotificationAsync(notificationId);
        await AsyncStorage.removeItem(`@agendafacil:notification:${agendamentoId}`);
